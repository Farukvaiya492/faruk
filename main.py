...
def send_like(uid: str, server_name: str = "BD"):
    """Send likes to a Free Fire UID using the new API logic."""
    try:
        response = requests.get(f"{API_URL}?uid={uid}&server_name={server_name}&key={API_KEY}", timeout=10)
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                return {
                    "nickname": data.get('nickname', 'Unknown User'),
                    "uid": data.get('uid', 'Unknown'),
                    "before": data.get('likes_before', '0'),
                    "added": data.get('added_likes', '0'),
                    "total": data.get('total_likes', '0'),
                    "status": "Success ✅"
                }
            else:
                return {"status": "Like not sent"}
