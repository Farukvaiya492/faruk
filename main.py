...
"""
        ...
        return "Error validating phone number. Try again?"
...
async def get_binance_ticker(symbol: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"""
━━━━━━•❅•°•❈•°•❅•━━━━━━
💹 24hr Ticker Data for {data['symbol']}
📅 Time: {datetime.now(timezone(timedelta(hours=6))).strftime('%Y-%m-%d %H:%M:%S +06')}
━━━━━━•❅•°•❈•°•❅•━━━━━━
💰 Last Price: {data.get('lastPrice', 'N/A')}
📈 Price Change (24h): {data.get('priceChange', 'N/A')}
📊 Price Change Percent: {data.get('priceChangePercent', 'N/A')}%
🔺 24h High Price: {data.get('highPrice', 'N/A')}
    ...
    async def checkmail_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_type == 'private' and user_id != ADMIN_USER_ID:
            ...
            try:
                ...
                if not mail_list:
                    await update.message.reply_text(f"No emails found in the inbox for {email}. Try again later?")
                    return
                ...
                await update.message.reply_text("Error checking email. Try again?")
...
All systems ready!
    ...
    async def setadmin_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_type == 'private' and user_id != ADMIN_USER_ID:
            ...
            if ADMIN_USER_ID == 0:
                ...
                await update.message.reply_text(f"Congratulations {username}, you are now the bot admin! User ID: {user_id}")
                logger.info(f"Admin set to user ID: {user_id}")
            else:
                await update.message.reply_text("Admin already set. Only the current admin can manage the bot.")
    ...
    async def api_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_type == 'private' and user_id != ADMIN_USER_ID:
            ...
            await update.message.reply_text("Gemini AI API disabled. Use other commands like /weather, /ipinfo, or /like!")
    ...
    async def setmodel_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_type == 'private' and user_id != ADMIN_USER_ID:
            ...
            await update.message.reply_text("Model selection disabled. Use other commands like /info or /weather!")
...
"""
    ...
    async def countryinfo_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not re.match(r'^[\x00-\x7F]*$', country_name):
            await update.message.reply_text("Please enter the country name in English. Example: 'Bangladesh'.")
            return
        ...
        if user_id != ADMIN_USER_ID:
            ...
            if last_like_time and (current_time - last_like_time).total_seconds() < 24 * 60 * 60:
                ...
                await update.message.reply_text(
                    ...
                    f"Please wait {hours_left} hours and {minutes_left} minutes."
                )
                return
    ...
    async def handle_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await context.bot.send_photo(
                chat_id=chat_id,
                photo=FREE_FIRE_LOGO_URL,
                caption="Error processing image. Try again!"
            )
            if chat_id in removebg_state:
                del removebg_state[chat_id]
