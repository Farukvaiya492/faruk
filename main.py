...
Conversation Style:
- Detect the user's language from the conversation and always respond in that same language. For example, if the user writes in Bengali, you must respond in Bengali.
...
- If the user sends a 2 or 3 letter lowercase word (e.g., "ki", "ke", "ken"), always provide a meaningful, friendly, and contextually relevant response in the user's language.
...
Respond as I Master Tools. Keep it natural, engaging, surprising, and match the conversation's tone. Respond in the user's language. Do NOT start the response with the user's name or phrases like "Oh" or "Hey".
"""
            model_to_use = coding_model if is_coding_query else general_model
